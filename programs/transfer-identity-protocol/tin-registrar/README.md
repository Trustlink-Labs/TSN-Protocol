...
After deployment, initialize the global registry state before issuing a
private TIN:

```bash
cd /path/to/trust-link
...
This is an idempotent Devnet operation. It creates the global-state PDA once;
it does not issue a TIN. A fresh private TIN must subsequently be created
through the SDK, Node, and generic Cranker flow.
