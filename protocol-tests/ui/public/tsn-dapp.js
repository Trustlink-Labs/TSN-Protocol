(() => {
    ...
    window.startTsnDapp = () => {
        ...
        document.querySelector('#scene').innerHTML = `<div class="tsn-hero"><div><div class="tsn-kicker">Developer sandbox</div><h2>TSN Wallet Dapp</h2><p class="small">Prepare identity, payment, and settlement actions with the SDK. Every signature and submission stays explicit.</p></div><div class="tsn-status">NETWORK<strong>DEVNET</strong></div></div><div class="cards">
            <div class="card tsn-card"><h3>Create a TIN intent</h3><p class="small">Create the owner-signed request that the TSN Node can verify.</p><label for="tinValue">TIN <input id="tinValue" inputmode="numeric" pattern="[0-9]{10}" placeholder="10-digit number" maxlength="10" autocomplete="off" required></label><label for="tinName">Display name <input id="tinName" placeholder="Alice" autocomplete="name" required></label><button id="prepareTin">Prepare and sign</button><p class="small">Direct TIP account creation is disabled by the SDK; this prepares the signed TSN operation.</p></div>
            <div class="card tsn-card"><h3>Wallet-to-wallet payment</h3><p class="small">Authorize a payment to a recipient TIN and submit it to the Receiver.</p><label for="paymentTin">Recipient TIN <input id="paymentTin" inputmode="numeric" pattern="[0-9]{10}" placeholder="10-digit number" maxlength="10" autocomplete="off" required></label><label for="routeCommitment">Latest route commitment <input id="routeCommitment" pattern="[0-9a-fA-F]{64}" placeholder="64-character hex commitment" autocomplete="off" required></label><label>Current route version <input id="routeVersion" value="1" readonly aria-readonly="true"></label><p class="small">Only the current route version is accepted. Older versions are retired and cannot be selected.</p><label for="paymentAmount">Amount <input id="paymentAmount" type="number" value="1" min="0.000001" step="0.000001" required></label><label for="paymentMint">Token mint <input id="paymentMint" value="${escapeHtml(window.trustlinkLabWallet?.selectedMint ?? '')}" placeholder="SPL mint" autocomplete="off" required></label><button id="preparePayment">Sign and submit intent</button></div>
        ...
        document.querySelector('#prepareTin').onclick = () => runValidated(['tinValue', 'tinName'], prepareTin);
        document.querySelector('#preparePayment').onclick = () => runValidated(['paymentTin', 'routeCommitment', 'routeVersion', 'paymentAmount', 'paymentMint'], preparePayment);
    ...
    async function prepareTin() {
        try { requireWallet(); const draft = await api('/api/tsn/sdk/prepare-tin', { method: 'POST', body: JSON.stringify({ tin: document.querySelector('#tinValue').value, displayName: document.querySelector('#tinName').value }) }); const signatureBase64 = await sign(draft.message); result({ ...draft, signatureBase64, next: 'POST the full encrypted TIN operation to the TSN Node /tin-operations endpoint.' }); } catch (error) { result(`BLOCKED: ${error.message}`); }
    }
    async function preparePayment() {
        try { const senderWallet = requireWallet(); const draft = await api('/api/tsn/sdk/prepare-payment', { method: 'POST', body: JSON.stringify({ recipientTin: document.querySelector('#paymentTin').value, recipientRouteCommitment: document.querySelector('#routeCommitment').value, routeVersion: document.querySelector('#routeVersion').value, amount: document.querySelector('#paymentAmount').value, tokenMintAddress: document.querySelector('#paymentMint').value }) }); const signatureBase64 = await sign(draft.message); const submitted = await api('/api/tsn/sdk/submit-payment', { method: 'POST', body: JSON.stringify({ ...draft, senderWallet, signatureBase64 }) }); result({ status: 'PAYMENT_INTENT_SUBMITTED', draft, submitted }); } catch (error) { result(`BLOCKED: ${error.message}`); }
    }
