...
No. Cross-network settlement uses a correspondent-bank model. Solana authorizes and holds the debit commitment. Creditcoin is the first live EVM settlement rail: the `CreditcoinSettlementHub` consumes the Node's EIP-712 authorization and pays the recipient from its own stablecoin liquidity. Other EVM destinations (for example Base and Ethereum) use `DestinationLiquidityRegistry` routes with proof-backed liquidity observations verified through Attestcoin. There are no wrapped tokens and no cross-chain message passing.
...
- Protocol and docs source: `trustlink-labs/tsn-protocol` (docs live under `tsn-docs/`)
