...
<Steps>
  ...
  <Step title="Node verifies and signs a permit">
    A Node checks signatures, TIN and GPRU bindings, sequence, expiry, and policy. It then signs a short-lived Ed25519 permit binding the settlement slot, commitments, nullifier, selected Cranker vault, recipient, mint, amount, and lease. A Node cannot sign as Mother, mutate the intent, or submit the transaction itself, so it can only authorize an exact leased operation or reject.
  </Step>
  ...
  <Step title="Mother materializes a one-time settlement slot">
    Mother submits `tsn_create_settlement_dna`, which writes a `SettlementDna` PDA at an opaque HMAC-SHA256 slot. The slot binds the settlement commitment, digest, nullifier, cranker vault, recipient, mint, amount, and lease. No payment id, sender, or route metadata is written on-chain.
  </Step>
  <Step title="Cranker submits the payout atomically">
    The Cranker pays Solana fees and submits `tsn_execute_private_payout` with the Node's Ed25519 permit. The TSN Program re-checks the stored `SettlementDna`, the permit, the lease, and the treasury liability. It then transfers from the epoch treasury and reimburses the cranker vault, atomically or not at all. The `EpochClaimSlot` PDA is created on this first settlement or refund attempt: first-writer-wins.
  </Step>
...
<AccordionGroup>
  <Accordion title="TIN and GPRU: identity and routing" icon="id-card">
    A **Transfer Identity Number (TIN)** is a 10-digit portable identity issued by the **Transfer Identity Protocol (TIP)**. The encrypted master seed is sealed to the owner wallet via a `wallet-owner-signature-v1` envelope and authorizes derivation of **GPRU** scopes. A GPRU scope is a non-custodial authorization and routing scope derived from the TIN privacy-receiving root, settlement commitment, epoch, and authorization scope. See [Identity and routing](/architecture/identity-and-routing).
  </Accordion>
