...
The root wallet is the TIN authority. New and upgraded TINs use a wallet-owned envelope. The owner signs a fresh, canonical TIN-access authorization on whichever device they are using. The SDK then derives a local data key from that approval to decrypt the envelope on-device.
