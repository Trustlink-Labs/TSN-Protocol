...
## TSN settlement path vs TCAP credit path

TSN and TCAP are separate on-chain paths, and they run in that order.

The TSN settlement path is the live payout. The Cranker submits `tsn_execute_private_payout`. This instruction re-verifies the Mother-issued `SettlementDna` and checks the Node's Ed25519 permit and lease. It then transfers the payout from the CrankerVault to the recipient, and reimburses the CrankerVault from the epoch treasury, atomically or not at all. This path does not touch a TCAP tip.

The TCAP credit path runs after the TSN payout. The Cranker submits `credit_tcap_tin_tip_v1` with `CreditTcapTinTipV1Args`. This instruction advances the `TCapTinTipV1` PDA from `previous_commitment` to `new_commitment` under a scoped GPRU authorization. This path does not touch the CrankerVault, `SettlementDna`, or the epoch treasury. A PDA-signed CPI from the approved TSN program authorizes the tip advance.

Neither path uses an `AcceptedIntentV1` PDA. That instruction exists but is not on the live privacy-safe credit path.
