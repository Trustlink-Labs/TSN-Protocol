...
The TSN settlement path is the live payout. The Cranker submits `tsn_execute_private_payout`, which re-verifies the Mother-issued `SettlementDna`, checks the Node's Ed25519 permit and lease, transfers the payout from the CrankerVault to the recipient, and reimburses the CrankerVault from the epoch treasury, atomically or not at all. This path does not touch a TCap tip.

The TCap credit path runs after the TSN payout. The Cranker submits `credit_tcap_tin_tip_v1` with `CreditTcapTinTipV1Args`, which advances the `TCapTinTipV1` PDA from `previous_commitment` to `new_commitment` under a scoped GPRU authorization. This path does not touch the CrankerVault, `SettlementDna`, or the epoch treasury; the tip advance is authorized by a PDA-signed CPI from the approved TSN program.
