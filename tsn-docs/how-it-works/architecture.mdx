...
**In plain English:** This is the canonical description of the current Transfer Settlement Network. A payment starts as a signed authorization on the sender's device. It then moves through TIN identity, GPRU authorization, TSN epoch treasury coordination, a Mother-rooted DNA permit, and a TCAP tip credit against an encrypted snapshot. There is no public accepted-intent record, no per-payment PDA, and no receipt joining funding to credit.
...
The Node/Mother keeps the payment binding (recipient route, sender refund destination, funding lineage, amount, mint, epoch, payment hash) in encrypted storage. The encryption key and plaintext never leave Node/Mother. Authorized Node/Mother derives a keyed slot with HMAC-SHA256 and withholds it until settlement or refund.
...
**First successful operation wins.** Settlement runs atomically. It checks the Node permit, lease, slot state, nullifier, and treasury liability. It then pays the recipient from the CrankerVault, reimburses that exact CrankerVault for the exact amount, marks the slot `SETTLED`, and records the successful Cranker. Refund uses the same slot and marks it `REFUNDED`, returning the exact amount from epoch treasury to the Node-authorized refund destination. A later operation against the same slot fails before token movement.
