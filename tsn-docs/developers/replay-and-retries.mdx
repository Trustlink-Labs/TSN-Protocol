...
Every field in the authorization is signed as a unit. Any tampering, including to amount, token, recipient, commitment, or expiry, invalidates the signature. The Node and on-chain program verify the complete binding before acceptance.
...
If the validity window expires, the work is no longer submittable. The sender must sign a new intent with a fresh nonce, nullifier, and slot window.
