...
<Note>
  **Status: Live on Devnet.** Anyone can run a Cranker against the deployed TSN
  program. Admission is permissionless: the Receiver checks the operator's
  on-chain Cranker PDA against the pinned TSN program's Mother DNA account. There
  is no per-operator allowlist and no Receiver-issued API key.
</Note>
...
Cranker admission is permissionless. The operator does not request access from the deployment owner. Instead, the operator registers its own Cranker PDA on Solana Devnet, and the Receiver admits any operator whose Cranker PDA is anchored to the pinned TSN program's Mother DNA account.

Before starting, the operator needs:

1. The pinned TSN program ID and matching build / IDL published by the deployment owner
2. The Receiver URL that exposes Cranker work endpoints
3. A reachable TSN Node that verifies and publishes work (operated by the deployment; not something the Cranker runs)
4. A registered on-chain Cranker PDA derived from the Mother Escrow and the operator public key on the pinned TSN program
5. Direct Solana Devnet RPC access for reads and submissions
...
The Receiver authenticates each Cranker request with a signed challenge from the operator keypair and then confirms the Cranker PDA on-chain. If the PDA is missing or does not resolve to the pinned Mother DNA, the Receiver returns `401` and no work is leased. There is no separate Cranker API key to rotate.
