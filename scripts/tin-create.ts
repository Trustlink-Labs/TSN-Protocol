...
import { createTipClient } from "../tsn-protocol/programs/transfer-identity-protocol/tip-sdk/src/index.js";
...
async function main() {
  ...
  console.log(
    `Setting up new TIN for '${displayName}' with phone '${phoneNumber}'...`,
  );
  ...
  try {
    ...
    console.log(
      `You can now run: npm run test:lookup ${result.tin.toString()}`,
    );
  } catch (error) {
