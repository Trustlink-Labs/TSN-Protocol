...
const rpcUrl = process.env.CREDITCOIN_RPC_URL?.trim() || "https://rpc.cc3-testnet.creditcoin.network";
const deployerFile = path.resolve(
  process.env.CREDITCOIN_DEPLOYER_KEYPAIR_FILE?.trim() ||
    path.join(protocolRoot, "tsn-crosschain", ".creds", "contract_auth_keypair.json"),
);
const crankerFile = path.resolve(
  process.env.CREDITCOIN_CRANKER_KEYPAIR_FILE?.trim() ||
    path.join(protocolRoot, "tsn-crosschain", ".creds", "creditcoin_cranker_keypair.json"),
);
...
async function main() {
  ...
  if (network.chainId !== 102031n) throw new Error(`Expected CC3 Testnet chain ID 102031, got ${network.chainId}`);

  if (fs.existsSync(crankerFile)) {
    const existing = JSON.parse(fs.readFileSync(crankerFile, "utf8")) as KeyFile;
    ...
    console.log(`Creditcoin Cranker balance: ${ethers.formatEther(await provider.getBalance(wallet.address))} CTC`);
    console.log(`Key file preserved: ${path.relative(protocolRoot, crankerFile)}`);
    if (existing.address && ethers.getAddress(existing.address) !== wallet.address) {
      throw new Error("Existing key file address does not match its private key");
    }
    return;
  }
  ...
  fs.writeFileSync(crankerFile, JSON.stringify({ address: cranker.address, privateKey: cranker.privateKey }, null, 2) + "\n", { mode: 0o600 });
  ...
  if (deployerBalance < value) throw new Error(`Deployer balance is below ${fundingAmount} CTC; no funding transaction sent`);
  ...
  if (!receipt || receipt.status !== 1) throw new Error(`Funding transaction was not confirmed successfully: ${tx.hash}`);
