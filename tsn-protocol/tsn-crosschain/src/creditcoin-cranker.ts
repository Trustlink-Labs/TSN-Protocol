...
} from "../../tsn-sdk/src/cross-chain.js";
import {
...
export class CreditcoinCranker {
  ...
  async submit(
    ...
    if (authorization.routeId.toLowerCase() !== evidence.route.routeId.toLowerCase()) {
      throw new Error("authorized route does not match the on-chain registry");
    }
    if (getAddress(authorization.destinationExecutor) !== getAddress(evidence.route.destinationExecutor)) {
      throw new Error("authorized executor does not match the on-chain registry");
    }
    if (getAddress(authorization.token) !== getAddress(evidence.route.settlementToken)) {
    ...
    if (!receipt || receipt.status !== 1) throw new Error("Creditcoin settlement transaction failed");
    const parsed = receipt.logs
      .map((log: Log): LogDescription | null => { try { return this.hub.interface.parseLog(log); } catch { return null; } })
      .find((event: LogDescription | null) => event?.name === "SettlementMessagePublished");
    if (!parsed) throw new Error("SettlementMessagePublished event was not emitted");
