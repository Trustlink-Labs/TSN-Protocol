...
function required(name: string): string {
  ...
  if (!value) throw new Error(`Missing ${name}; no real integration evidence can be verified`);
  return value;
}
...
async function solanaSignatureStatus(signature: string): Promise<Record<string, unknown>> {
  if (!/^[1-9A-HJ-NP-Za-km-z]{32,100}$/.test(signature)) {
    throw new Error("SOLANA_INTENT_TX_SIGNATURE is not a valid Solana signature format");
  }
  ...
  const body = await response.json() as { error?: unknown; result?: { value?: Array<Record<string, unknown> | null> } };
  if (body.error) throw new Error(`Solana RPC error: ${JSON.stringify(body.error)}`);
  const status = body.result?.value?.[0];
  if (!status) throw new Error("Solana intent signature was not found");
  if (status.err !== null) throw new Error(`Solana intent failed: ${JSON.stringify(status.err)}`);
  return status;
}
...
async function main(): Promise<void> {
  ...
  if (network.chainId !== cc3ChainId) throw new Error(`Expected CC3 chain ID ${cc3ChainId}, got ${network.chainId}`);
  ...
  if (!nodeResponse.ok) throw new Error(`Node route gate returned HTTP ${nodeResponse.status}`);
  const routes = await nodeResponse.json();
  if (!Array.isArray(routes) || routes.length === 0) throw new Error("Node route gate returned no configured destination routes");
  ...
  const creditcoinHash = assertHash(required("CREDITCOIN_SETTLEMENT_TX_HASH"), "CREDITCOIN_SETTLEMENT_TX_HASH");
  console.log("Checking Creditcoin settlement transaction ...");
  const receipt = await provider.getTransactionReceipt(creditcoinHash);
  if (!receipt) throw new Error("Creditcoin settlement transaction is not mined");
  if (receipt.status !== 1) throw new Error("Creditcoin settlement transaction failed");
  if (!receipt.to || getAddress(receipt.to) !== hubAddress) throw new Error(`Creditcoin transaction target is not the deployed Settlement Hub: ${receipt.to}`);
  const published = receipt.logs.map((log) => { try { return hubInterface.parseLog(log); } catch { return null; } }).find((event) => event?.name === "SettlementMessagePublished");
  if (!published) throw new Error("SettlementMessagePublished event was not found");

  console.log(`Creditcoin settlement confirmed: ${receipt.hash}`);
  console.log(`SettlementMessagePublished event: ${JSON.stringify(published.args)}`);
  console.log("Real cross-chain evidence verified. No transaction was submitted by this verifier.");
}
