...
use tcap::instruction::CreditOneTimeTipTransfer;
...
pub struct RegisterTcapOneTimeTransferCreditArgs {
...
#[derive(Accounts)]
pub struct RegisterTcapOneTimeTransferCredit<'info> {
...
pub fn handler(ctx: Context<RegisterTcapOneTimeTransferCredit>, args: RegisterTcapOneTimeTransferCreditArgs) -> Result<()> {
    ...
    let data = CreditOneTimeTipTransfer::data(&tcap::instruction::CreditOneTimeTipTransfer {
      args: tcap::instructions::credit_one_time_tip_transfer::CreditOneTimeTipTransferArgs {
    ...
    let ix = Instruction { program_id: TCAP_PROGRAM_ID, accounts: vec![
        ...
        AccountMeta::new(ctx.accounts.current_tip.key(), false),
        AccountMeta::new_readonly(ctx.accounts.asset_entry.key(), false),
        AccountMeta::new(ctx.accounts.reserve_state.key(), false),
        AccountMeta::new(ctx.accounts.liability.key(), false),
    ...
    invoke_signed(&ix, &[
        ...
        ctx.accounts.tsn_program.to_account_info(), ctx.accounts.tcap_authorization_signer.to_account_info(), ctx.accounts.tcap_program.to_account_info(),
