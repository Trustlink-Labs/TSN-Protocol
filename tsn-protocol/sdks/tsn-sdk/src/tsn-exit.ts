...
export const TSN_PROGRAM_ID = new PublicKey(
...
export function getTcapProgramId(): PublicKey {
  ...
  if (!value)
    throw new Error("TCAP_PROGRAM_ID is required for TSN exit instructions");
  try {
...
function bytes32(
  value: Uint8Array | string | Buffer,
  label: string,
): Uint8Array {
  const bytes =
...
export function buildExitDebitInstruction(
  params: ExitDebitParams,
): TransactionInstruction {
  ...
  return new TransactionInstruction({
    ...
    keys: [
      ...
      {
        pubkey: pubkey(params.motherEscrow),
        isSigner: false,
        isWritable: false,
      },
...
export function buildExitDebitTransaction(
  params: ExitDebitParams,
): Transaction {
...
export function buildExitPayoutInstruction(
  params: ExitPayoutParams,
): TransactionInstruction {
  ...
  return new TransactionInstruction({
    ...
    keys: [
      ...
      {
        pubkey: pubkey(params.reserveAuthority),
        isSigner: false,
        isWritable: false,
      },
      ...
      {
        pubkey: pubkey(params.tokenProgram ?? TOKEN_PROGRAM_ID),
        isSigner: false,
        isWritable: false,
      },
...
export function buildExitPayoutTransaction(
  params: ExitPayoutParams,
): Transaction {
  return new Transaction().add(buildExitPayoutInstruction(params));
}

export function deriveExitPermitPda(input: {
  permitNonce: Uint8Array | string;
}): PublicKey {
