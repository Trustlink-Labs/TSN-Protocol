...
import {
  RecurringProvider,
  assertVerifiedRecurringProvider,
  providerSupportsTokenMint,
} from "./provider.js";

export type RecurringMandateStatus =
  | "PENDING"
  | "ACTIVE"
  | "SUSPENDED"
  | "REVOKED"
...
function assertIsoTimestamp(value: unknown, fieldName: string): void {
  ...
  if (Number.isNaN(parsed)) {
    throw new RecurringMandateValidationError(
      `${fieldName} must be a valid ISO timestamp string.`,
    );
  }
}
...
export function validateRecurringMandate(
  ...
  if (!mandate || typeof mandate !== "object") {
    throw new RecurringMandateValidationError(
      "Mandate record must be a valid object.",
    );
  }

  if (!mandate.mandateId || typeof mandate.mandateId !== "string") {
    throw new RecurringMandateValidationError(
      "mandateId is required and must be a string.",
    );
  }

  if (mandate.providerId !== provider.providerId) {
    throw new RecurringMandateValidationError(
      "mandate providerId must match the verified provider.",
    );
  }

  if (
    !mandate.subscriberCommitment ||
    typeof mandate.subscriberCommitment !== "string"
  ) {
    throw new RecurringMandateValidationError(
      "subscriberCommitment is required and must be a string.",
    );
  }

  if (
    !mandate.sourceCommitment ||
    typeof mandate.sourceCommitment !== "string"
  ) {
    throw new RecurringMandateValidationError(
      "sourceCommitment is required and must be a string.",
    );
  }

  if (!mandate.assetCommitment || typeof mandate.assetCommitment !== "string") {
    throw new RecurringMandateValidationError(
      "assetCommitment is required and must be a string.",
    );
  }

  if (
    mandate.policyCommitment != null &&
    typeof mandate.policyCommitment !== "string"
  ) {
    throw new RecurringMandateValidationError(
      "policyCommitment must be a string when provided.",
    );
  }

  if (
    !mandate.tokenMintAddress ||
    typeof mandate.tokenMintAddress !== "string"
  ) {
    throw new RecurringMandateValidationError(
      "tokenMintAddress is required and must be a string.",
    );
  }

  if (!providerSupportsTokenMint(provider, mandate.tokenMintAddress)) {
    throw new RecurringMandateValidationError(
      "Provider does not support the requested token mint.",
    );
  }

  if (
    typeof mandate.maxChargeAmountBaseUnits !== "bigint" ||
    mandate.maxChargeAmountBaseUnits <= 0n
  ) {
    throw new RecurringMandateValidationError(
      "maxChargeAmountBaseUnits is required and must be a positive bigint.",
    );
  }

  if (
    !Number.isInteger(mandate.minIntervalSeconds) ||
    mandate.minIntervalSeconds <= 0
  ) {
    throw new RecurringMandateValidationError(
      "minIntervalSeconds is required and must be a positive integer.",
    );
  }

  if (
    mandate.maxExecutions != null &&
    (!Number.isInteger(mandate.maxExecutions) || mandate.maxExecutions <= 0)
  ) {
    throw new RecurringMandateValidationError(
      "maxExecutions must be a positive integer when provided.",
    );
  }

  if (
    mandate.maxCumulativeAmountBaseUnits != null &&
    (typeof mandate.maxCumulativeAmountBaseUnits !== "bigint" ||
      mandate.maxCumulativeAmountBaseUnits <= 0n)
  ) {
    throw new RecurringMandateValidationError(
      "maxCumulativeAmountBaseUnits must be a positive bigint when provided.",
    );
  }
  ...
  if (Date.parse(mandate.startTime) >= Date.parse(mandate.expiryTime)) {
    throw new RecurringMandateValidationError(
      "startTime must be before expiryTime.",
    );
  }
  ...
  if (!mandate.executionNonce || typeof mandate.executionNonce !== "string") {
    throw new RecurringMandateValidationError(
      "executionNonce is required and must be a string.",
    );
  }

  if (!mandate.createdAt || typeof mandate.createdAt !== "string") {
    throw new RecurringMandateValidationError(
      "createdAt is required and must be an ISO timestamp string.",
    );
  }

  if (!mandate.updatedAt || typeof mandate.updatedAt !== "string") {
    throw new RecurringMandateValidationError(
      "updatedAt is required and must be an ISO timestamp string.",
    );
  }
...
export function buildRecurringMandateCanonicalMessage(
  mandate: RecurringMandate,
): string {
  const fields = [
    ...
    [
      "maxExecutions",
      mandate.maxExecutions != null ? mandate.maxExecutions.toString() : "",
    ],
    [
      "maxCumulativeAmountBaseUnits",
      mandate.maxCumulativeAmountBaseUnits != null
        ? mandate.maxCumulativeAmountBaseUnits.toString()
        : "",
    ],
...
export function computeRecurringMandateCommitment(
  mandate: RecurringMandate,
): string {
