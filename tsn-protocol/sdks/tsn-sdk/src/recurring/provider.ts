...
export type RecurringProviderStatus =
  | "PENDING"
  | "ACTIVE"
  | "SUSPENDED"
  | "REVOKED";
...
export function validateRecurringProvider(provider: RecurringProvider): void {
  if (!provider || typeof provider !== "object") {
    throw new RecurringProviderValidationError(
      "Provider record must be a valid object.",
    );
  }
  ...
  if (!provider.providerPda || typeof provider.providerPda !== "string") {
    throw new RecurringProviderValidationError(
      "providerPda is required and must be a string.",
    );
  }
  ...
  if (typeof provider.recurringEnabled !== "boolean") {
    throw new RecurringProviderValidationError(
      "recurringEnabled is required and must be a boolean.",
    );
  }

  if (
    !Array.isArray(provider.supportedTokenMints) ||
    provider.supportedTokenMints.some((mint) => typeof mint !== "string")
  ) {
    throw new RecurringProviderValidationError(
      "supportedTokenMints must be an array of token mint addresses.",
    );
  }

  if (
    !provider.routeCommitment ||
    typeof provider.routeCommitment !== "string"
  ) {
    throw new RecurringProviderValidationError(
      "routeCommitment is required and must be a string.",
    );
  }

  if (!provider.verifiedAt || typeof provider.verifiedAt !== "string") {
    throw new RecurringProviderValidationError(
      "verifiedAt is required and must be an ISO timestamp string.",
    );
  }
  ...
  if (
    provider.providerId === TSN_TEST_PROVIDER_ID &&
...
export function assertVerifiedRecurringProvider(
  ...
  if (provider.status !== "ACTIVE") {
    throw new RecurringProviderValidationError(
      "Provider must be ACTIVE to support recurring mandates.",
    );
  }

  if (!provider.recurringEnabled) {
    throw new RecurringProviderValidationError(
      "Provider must have recurring permission enabled.",
    );
  }
}

export function providerSupportsTokenMint(
  provider: RecurringProvider,
  tokenMint: string,
): boolean {
