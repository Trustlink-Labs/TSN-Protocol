...
<Note>
  Transport does not authorize, and authorization does not require transport tobe honest. Mother Nodes, Receivers, and Cranker Nodes move traffic through TSN, but they cannot forge a Mother signature, rewrite sender-signed fields, or replay asettlement.
</Note>
...
<Steps>
  ...
  <Step title="Node verifies">
    A Node checks signatures, TIN and GPRU bindings, sequence, expiry,and policy, then signs a short-lived Ed25519 permit binding the settlement slot, commitments, nullifier, selected Cranker vault, recipient, mint, amount, and lease. A Node cannot sign as Mother, mutate the intent, or submit the transaction itself, so it can only authorize an exact leased operation orreject.
  </Step>
